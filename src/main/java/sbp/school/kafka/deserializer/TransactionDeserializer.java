...
public class TransactionDeserializer implements Deserializer<TransactionDto> {
    ...
    public TransactionDto deserialize(String topic, byte[] data) {
        ...
        try {
            transaction = objectMapper.readValue(data, TransactionDto.class);
        } catch (IOException e) {
        ...
        if (!JsonSchemeValidator.isObjectValid(transaction, "/transaction_schema.json")) {
            logger.error(String.format(invalidFormat, transaction.getTransaction().getUuid()));

            throw new InvalidFormatException(String.format(invalidFormat, transaction.getTransaction().getUuid()));
        }
